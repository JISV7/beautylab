...
const SortIcon = ({ column, sortConfig }: { column: SortConfig['column']; sortConfig: SortConfig }) => {
...
export const FontDataTable: React.FC<FontDataTableProps> = ({
    ...
    const getDeleteMessage = (font: Font) => {
        ...
        if (usageCount > 0) {
            const usageText = usage.map((u: { themeName: string; element: string }) => `${u.themeName} (${u.element})`).join(', ');
            return (
                <>

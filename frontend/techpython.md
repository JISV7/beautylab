...
Read the file technologiespy.md
...
*   **Backend Manager**: pip / uv (Python 3.12+).
...
### 6.1. Required Package Adjustments
Based on current installation, the following are recommended for completion:
*   `python-jose[cryptography]`: For JWT encoding/decoding.
*   `asyncpg`: Async database driver for PostgreSQL + SQLAlchemy.
*   `passlib`: Convenience layer for Argon2 hashing (optional, `argon2-cffi` works directly).
